      ...
      <div className="w-full max-w-none px-2 sm:px-4 py-4 sm:py-8 mx-auto">
        ...
        <div className="w-full max-w-[600px] mx-auto mb-4 sm:mb-6">
          ...
          gridTemplateColumns: '30% 70%'
            ...
            <Button onClick={() => setShowAccessCodeValidation(true)} style={{
        ...
        {!accessCode && temporaryStorage.isSessionActive() && <div className="w-full max-w-[600px] mx-auto mb-4">
        ...
        <div className="w-full max-w-[600px] mx-auto">
          <div className="glass-container">
            ...
            <MenstrualCycleTracker accessCode={accessCode} onFirstInteraction={handleFirstInteraction} />
          </div>
        </div>
                  ...
                  <DialogContent className="glass-container border-0 backdrop-blur-xl shadow-2xl max-w-xs mx-2 max-h-[80vh] overflow-hidden">
                    <DialogHeader className="pb-2 flex-shrink-0">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => setShowMonthlyCalendar(false)} className="text-muted-foreground hover:text-foreground h-8 w-8 p-0">
                          Zatvoriť
                        </Button>
                      </div>
                    </DialogHeader>
                    <div className="px-0 overflow-y-auto flex-1 min-h-0">
                      <MonthlyCalendar habitData={habitData} selectedMonth={monthlyCalendarDate} onMonthChange={setMonthlyCalendarDate} habits={habits} formatDate={habitFormatDate} />
                    </div>
                  </DialogContent>
                </Dialog>
              </div>
            </div>
            <HabitTracker selectedDate={selectedDate} onFirstInteraction={handleFirstInteraction} />
          </div>
        </div>
        
        {/* Reflection Widget */}
        <div className="w-full max-w-[600px] mx-auto">
          <div className="glass-container text-base">
            <div className="flex items-center justify-between mb-4">
                ...
                <Dialog open={showDiaryView} onOpenChange={setShowDiaryView}>
                  ...
                  <DialogContent className="max-w-2xl">
                    <DialogHeader className="pb-0">
                     ...
                     <DiaryView reflections={accessCode ? reflections as Record<string, any> : {}} formatDate={habitFormatDate} />
                  </DialogContent>
                </Dialog>
              </div>
            </div>
            <ReflectionWidget selectedDate={selectedDate} onFirstInteraction={handleFirstInteraction} />
          </div>
        </div>
        
        {!accessCode && !shouldOfferBiometric() && <div className="w-full max-w-[600px] mx-auto mt-4">
            <div className="text-center p-4 bg-muted/50 rounded-lg border border-border/50">
              <div className="space-y-3">
                <Button onClick={handleEnterCodeClick} className="w-full">
        ...
        <AccessCodeSettings open={showAccessCodeSettings} onOpenChange={setShowAccessCodeSettings} />
        
        <AccessCodeValidation open={showAccessCodeValidation} onOpenChange={setShowAccessCodeValidation} onValidCode={handleValidAccessCode} />
        
        <PurchaseGatedBiometricWelcome open={showBiometricSetup} onOpenChange={setShowBiometricSetup} validatedCode={validatedAccessCode} />
        
        <BiometricPrompt open={showBiometricPrompt} onOpenChange={setShowBiometricPrompt} onSuccess={handleBiometricSuccess} onFallbackToCode={handleEnterCodeClick} />
        
        <AccessCodeInput open={showCodeInput} onOpenChange={setShowCodeInput} />

        <SaveProgressDialog isOpen={showSaveProgressDialog} onSave={() => {
