  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', minHeight: PILLAR_CARD_MIN_H, minWidth: 0, padding: '14px 15px 14px 126px', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
        ...
        >
          {p.titleParts
            ? <>{p.titleParts.before}{' '}<strong style={{ color: GOLD, fontWeight: 700 }}>{p.titleParts.em}</strong><span style={{ fontFamily: SANS, fontSize: '0.72em', fontWeight: 600, color: GOLD, marginLeft: 3 }}>{p.titleParts.num}</span></>
            : p.title}
        </div>
        {p.sub && (
