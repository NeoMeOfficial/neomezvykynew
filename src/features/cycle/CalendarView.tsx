...
export function CalendarView({
  ...
  return <div className="space-y-4">
      ...
      <div className={isMobile ? "space-y-3" : ""}>
        ...
        {/* Calendar Actions - Filter and Dropdown */}
          <div className={`flex gap-2 items-center ${isMobile ? '' : 'order-2 ml-auto'}`}>
            ...
            >
              <SelectTrigger className="w-[180px] rounded-lg border-[#FF7782] text-[#955F6A] hover:bg-[#FF7782]/10 focus:ring-[#FF7782]">
      ...
      <div className="space-y-[2px]">
        {/* Day Headers */}
        <div className={`grid grid-cols-7 ${isMobile ? 'gap-x-4 gap-y-[2px]' : 'gap-x-3 gap-y-[2px]'}`}>
      ...
      <Dialog open={!!selectedDayData} onOpenChange={(open) => !open && setSelectedDayData(null)}>
        <DialogContent 
          className={`${isMobile ? 'w-full max-w-[95%] max-h-[85vh] overflow-y-auto p-0' : 'sm:max-w-lg'} !bg-transparent border-0`}
          style={{
