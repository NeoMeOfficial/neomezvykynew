/**
 * NeoMe — Programmes Data
 *
 * Single source of truth for the four canonical Telo programmes Gabi has authored:
 *   1. postpartum    — Level 1 — 8 weeks
 *   2. bodyforming   — Level 2 — 6 weeks
 *   3. elastic-bands — Level 3 — 6 weeks
 *   4. strong-sexy   — Level 4 — 6 weeks
 *
 * Equipment items and FAQ answers contain inline HTML links (e.g. SharpShape
 * purchase URLs). Renderers must use `dangerouslySetInnerHTML` for these
 * fields to preserve the links.
 *
 * Extracted from src/pages/v2/PostpartumInfo.tsx on 2026-04-26 so the new
 * Telo screen designs can pull from one place.
 */
...
export interface ProgramPhase {
  weeks: string;        // e.g. "Týždeň 1"
  title: string;        // e.g. "Budujeme základy"
  description: string;
  icon: string;         // emoji marker — design may swap this for a token icon
}

export interface ProgramFAQ {
  q: string;
  a: string;            // may contain HTML (links) — render with dangerouslySetInnerHTML
}

export type ProgramSlug = 'postpartum' | 'bodyforming' | 'elastic-bands' | 'strong-sexy';
...
export interface Program {
  ...
  phases?: ProgramPhase[];
  equipment: string[];  // each entry may contain inline HTML — render with dangerouslySetInnerHTML
  faqs: ProgramFAQ[];
}

// Order is canonical — programmes appear in this order on the Telo list.
export const PROGRAM_SLUGS: ProgramSlug[] = ['postpartum', 'bodyforming', 'elastic-bands', 'strong-sexy'];
...
export const programs: Record<ProgramSlug, Program> = {
  'postpartum': {
    ...
    weeks: 8,
    description:
      'Program je vhodný pre ženy, ktoré potrebujú spevniť brušný korzet, vyriešiť diastázu či inkontinenciu, mesiace aj roky po pôrode.',
    features: [
    ...
    phases: [
      { weeks: 'Týždeň 1', title: 'Budujeme základy',                              description: '', icon: '🌱' },
      { weeks: 'Týždeň 2', title: 'Aktivácia panvového dna',                       description: '', icon: '🎯' },
      { weeks: 'Týždeň 3', title: 'Hlboké brušné svaly TVA',                       description: '', icon: '💪' },
      { weeks: 'Týždeň 4', title: 'Čo s bolesťou chrbta',                          description: '', icon: '🩺' },
      { weeks: 'Týždeň 5', title: 'Posilňovanie s gumou',                          description: '', icon: '💯' },
      { weeks: 'Týždeň 6', title: 'Viac než len sixpack',                          description: '', icon: '⭐' },
      { weeks: 'Týždeň 7', title: 'Tvoje telo pracuje ako celok',                  description: '', icon: '🔗' },
      { weeks: 'Týždeň 8', title: 'Prepojenie komplexnejších cvikov s dychom',     description: '', icon: '✨' },
    ],
    equipment: [
  ...
  'bodyforming': {
    ...
    weeks: 6,
    description:
      'Program je vhodný pre ženy, ktoré začínajú so spevňovaním celého tela a netrpia diastázou.',
    features: [
  ...
  'elastic-bands': {
    ...
    weeks: 6,
    description:
      'Program je vhodný pre ženy, ktoré chcú vyformovať postavu a zvýšiť intenzitu cvičenia s použitím dynamického odporu elastických gúm.',
    features: [
  ...
  'strong-sexy': {
    ...
    weeks: 6,
    description:
      'Program je vhodný pre ženy, ktoré chcú posunúť svoje hranice, získať silnejšie a vyformovanejšie telo a začať cvičiť s jednoručkami.',
    features: [
...
export const programList: Program[] = PROGRAM_SLUGS.map((slug) => programs[slug]);

export function getProgramBySlug(slug: string): Program | undefined {
  return (programs as Record<string, Program | undefined>)[slug];
}
