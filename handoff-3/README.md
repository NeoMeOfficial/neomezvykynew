...
├── ENGINEERING-RESPONSE.md          ← reply to eng's cross-check questions
├── 00-prototype/
│   └── NeoMe-Final-Review.html      single-file prototype, works offline
│                                    (open in browser, all 107 screens)
...
└── 04-screens/                      ← trial batch + per-screen prompts
    ...
    │   └── hero-yoga.jpg            editorial hero photo for welcome page
    ├── settings/                    Hub + Notifications
    ├── reference/                   PNG snapshots of trial-batch screens
    │   └── README.md                what each PNG is + matching artboard ID
    └── REFERENCE-VISUALS.md         where to find PNGs / live prototype
```

---

## For the engineering team's cross-check ask

Read `ENGINEERING-RESPONSE.md` — it answers the two open questions
(canonical `RitualCard` prop API, and the four "missing" UI atoms
`DatePicker` / `NumberStepper` / `ToggleRow` / `TopBar`), and points at the
single-file prototype + reference PNGs you can open side-by-side with your
running build.
